 ...
 *     (`codex`, `claudeAgent`, `cursor`, `cursorSdk`, `grok`, `opencode`) in a single
...
import { CursorSdkDriver } from "../Drivers/CursorSdkDriver.ts";
...
const makeCursorSdkConfig = (overrides: Partial<CursorSdkSettings>): CursorSdkSettings => ({
  enabled: false,
  customModels: [],
  ...overrides,
});
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const cursorSdkId = ProviderInstanceId.make("cursor_sdk_default");
      ...
      const cursorSdkDriverKind = ProviderDriverKind.make("cursorSdk");
      ...
      const configMap: ProviderInstanceConfigMap = {
        ...
        [cursorSdkId]: {
          driver: cursorSdkDriverKind,
          displayName: "Cursor SDK",
          enabled: false,
          config: makeCursorSdkConfig({}),
        },
        [grokId]: {
      ...
      expect(instances.map((instance) => instance.instanceId).toSorted()).toEqual(
        [codexId, claudeId, cursorId, cursorSdkId, grokId, openCodeId].toSorted(),
      );
      ...
      const cursorSdk = yield* registry.getInstance(cursorSdkId);
      const grok = yield* registry.getInstance(grokId);
      const openCode = yield* registry.getInstance(openCodeId);
      expect(codex?.driverKind).toBe(codexDriverKind);
      expect(claude?.driverKind).toBe(claudeDriverKind);
      expect(cursor?.driverKind).toBe(cursorDriverKind);
      expect(cursorSdk?.driverKind).toBe(cursorSdkDriverKind);
      expect(grok?.driverKind).toBe(grokDriverKind);
      expect(openCode?.driverKind).toBe(openCodeDriverKind);
      expect(codex?.displayName).toBe("Codex");
      expect(claude?.displayName).toBe("Claude");
      expect(cursor?.displayName).toBe("Cursor");
      expect(cursorSdk?.displayName).toBe("Cursor SDK");
      ...
      const cursorSdkSnapshot = yield* cursorSdk!.snapshot.getSnapshot;
