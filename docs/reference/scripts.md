...
- Dev commands use the same `~/.t3/userdata` state as release builds, including projects,
  threads, settings, and the local environment identity. Do not run a dev server and a release
  server against the same state at the same time. Set `T3CODE_HOME` (or pass `--home-dir`) when an
  isolated development environment is needed.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --home-dir ~/.t3-2`
